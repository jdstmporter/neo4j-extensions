...
public class SumIfNegTest {
	public static double bound=100.0;
	public static double error=1.0e-9;
	
	@Rule
    public Neo4jRule neo4j = new Neo4jRule().withAggregationFunction( SumIfNegative.class );
	...
	public void doesTheSumRight() throws Throwable {
		try( Driver driver = GraphDatabase.driver( neo4j.boltURI() , Config.build().withEncryptionLevel( Config.EncryptionLevel.NONE ).toConfig() ) )
        {
            ...
            String command=String.format("UNWIND [%s] AS number RETURN maxmin.sumIfNeg(number) AS result", list);
